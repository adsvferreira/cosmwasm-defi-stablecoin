...
class ArgumentsParser {
    ...
    static cLAToParamName(cLA) {
        if (cLA.toLowerCase() !== cLA) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, {
                param: cLA
            });
        }
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (taskName === undefined) {
                ...
                if (!this._isCLAParamName(arg, paramDefs)) {
                    throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_COMMAND_LINE_ARG, { argument: arg });
                }
            }
            else {
                if (!this._isCLAParamName(arg, paramDefs)) {
    ...
    // eslint-disable-next-line
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
                throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, {
                    param: arg
                });
            }
    ...
    _addTaskDefaultArguments(taskDefinition, taskArguments) {
        for (const paramName of Object.keys(taskDefinition.paramDefinitions)) {
            ...
            if (!definition.isOptional) {
                throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.MISSING_TASK_ARGUMENT, {
    ...
    _parseArgumentAt(rawCLAs, index, paramDefinitions, parsedArguments) {
        ...
        if (parsedArguments[paramName] !== undefined) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.REPEATED_PARAM, {
                param: claArg
            });
        }
    ...
    _parsePositionalParamArgs(rawPositionalParamArgs, positionalParamDefinitions // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        for (let i = 0; i < positionalParamDefinitions.length; i++) {
            ...
            if (rawArg === undefined) {
                if (!definition.isOptional) {
                    throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, {
        ...
        if (!hasVariadicParam &&
            ...
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, {
