...
const wasmkit_params_1 = require("../core/params/wasmkit-params");
...
const log = (0, debug_1.default)("wamkit:core:cli");
...
function ensureValidNodeVersion(packageJson) {
    ...
    if (!semver_1.default.satisfies(process.version, requirement)) {
        throw new errors_1.WasmkitError(errors_list_1.ERRORS.GENERAL.INVALID_NODE_VERSION, {
            requirement
        });
    }
}
function printErrRecur(error) {
    if (error.parent) {
        if (error.parent instanceof errors_1.WasmkitError) {
...
async function gatherArguments() {
    ...
    const envVariableArguments = (0, env_variables_1.getEnvRuntimeArgs)(wasmkit_params_1.WASMKIT_PARAM_DEFINITIONS, process.env);
    const argumentsParser = new arguments_parser_1.ArgumentsParser();
    const { runtimeArgs, taskName: maybeTaskName, unparsedCLAs } = argumentsParser.parseRuntimeArgs(wasmkit_params_1.WASMKIT_PARAM_DEFINITIONS, wasmkit_params_1.WASMKIT_SHORT_PARAM_SUBSTITUTIONS, envVariableArguments, process.argv.slice(2));
    if (runtimeArgs.verbose) {
        debug_1.default.enable("wasmkit*");
    }
...
async function loadEnvironmentAndArgs(maybeTaskName, runtimeArgs, argumentsParser, unparsedCLAs) {
    const ctx = context_1.WasmkitContext.createWasmkitContext();
    ...
    if (taskDefinitions[taskName] == null) {
        throw new errors_1.WasmkitError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_TASK, {
            task: taskName
        });
    }
    ...
    if (!isSetup && !(0, project_structure_1.isCwdInsideProject)()) {
        throw new errors_1.WasmkitError(errors_list_1.ERRORS.GENERAL.NOT_INSIDE_PROJECT, { task: origTaskName });
    }
...
async function main() {
    ...
    try {
        ...
        if (errors_1.WasmkitError.isWasmkitError(error)) {
            console.error(chalk_1.default.red(`Error ${error.message}`));
        }
        else if (errors_1.WasmKitPluginError.isWasmKitPluginError(error)) {
