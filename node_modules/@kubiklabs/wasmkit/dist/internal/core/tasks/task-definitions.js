...
const wasmkit_params_1 = require("../params/wasmkit-params");
/**
...
class SimpleTaskDefinition {
    ...
    constructor(name, isInternal = false) {
        ...
        this.action = () => {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET, {
                taskName: name
            });
        };
    }
    ...
    addParam(name, description, defaultValue, type, isOptional = defaultValue !== undefined) {
        if (type === undefined) {
            ...
            if (typeof defaultValue !== "string") {
                throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.DEFAULT_VALUE_WRONG_TYPE, {
                    paramName: name,
                    taskName: this.name
                });
            }
    ...
    _validateNotAfterVariadicParam(name) {
        if (this._hasVariadicParam) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateNameNotUsed(name) {
        if (this._hasParamDefined(name)) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED, {
                paramName: name,
                taskName: this.name
            });
        }
        if (Object.keys(wasmkit_params_1.WASMKIT_PARAM_DEFINITIONS).includes(name)) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.PARAM_CLASHES_WITH_WASMKIT_PARAM, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateNoMandatoryParamAfterOptionalOnes(name, isOptional) {
        if (!isOptional && this._hasOptionalPositionalParam) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.MANDATORY_PARAM_AFTER_OPTIONAL, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateParamNameCasing(name) {
        ...
        if (match === null) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.INVALID_PARAM_NAME_CASING, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    isOptional, name) {
        if (defaultValue !== undefined && !isOptional) {
            throw new errors_1.WasmkitError(errors_list_1.ERRORS.TASK_DEFINITIONS.DEFAULT_IN_MANDATORY_PARAM, {
                paramName: name,
                taskName: this.name
            });
        }
...
class OverriddenTaskDefinition {
    ...
    _throwNoParamsOverrideError(errorDescriptor) {
        throw new errors_1.WasmkitError(errorDescriptor, {
            taskName: this.name
        });
    }
}
