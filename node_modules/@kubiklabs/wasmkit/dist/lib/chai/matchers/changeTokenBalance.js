...
accountAddr, token, logResponse) {
    ...
    const balanceBefore = extractTokenBalance(await (0, client_1.getBalance)(client, accountAddr, env.network), token);
    const txResponse = await transaction();
    if (logResponse === true) {
        console.log(`${chalk_1.default.green("Transaction response:")} ${txResponse}`);
    }
    const balanceAfter = extractTokenBalance(await (0, client_1.getBalance)(client, accountAddr, env.network), token);
