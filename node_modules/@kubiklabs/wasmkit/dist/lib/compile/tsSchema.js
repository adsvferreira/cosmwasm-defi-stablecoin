...
const w = __importStar(require("@kubiklabs/wasmkit-ast-types"));
const wasmkit_ast_types_1 = require("@kubiklabs/wasmkit-ast-types");
...
outPath, skipSchemaErrors) {
    ...
    if (Object.prototype.hasOwnProperty.call(typeHash, 'Coin')) {
        body.push(w.importStmt(['Contract', 'wasmKitTypes'], '@kubiklabs/wasmkit'));
    }
    else {
        body.push(w.importStmt(['Contract', 'wasmKitTypes', 'Coin'], '@kubiklabs/wasmkit'));
    }
    // TYPES
    ...
    if (QueryMsg) {
        ...
        const queryInterface = await w.createQueryInterface(ReadOnlyInstance, QueryMsg, // eslint-disable-line  @typescript-eslint/no-explicit-any
        skipSchemaErrors);
        body.push(queryInterface);
        const queryClass = await w.createQueryClass(QueryClient, ReadOnlyInstance, "Contract", QueryMsg, // eslint-disable-line  @typescript-eslint/no-explicit-any
        skipSchemaErrors);
        body.push(queryClass);
    }
    // execute messages
    if (ExecuteMsg) {
        const children = (0, wasmkit_ast_types_1.getMessageProperties)(ExecuteMsg); // eslint-disable-line  @typescript-eslint/no-explicit-any
        if (children.length > 0) {
            Client = (0, case_1.pascal)(`${name}Contract`);
            Instance = (0, case_1.pascal)(`${name}Interface`);
            const executeInterface = await w.createExecuteInterface(Instance, ReadOnlyInstance, ExecuteMsg, // eslint-disable-line  @typescript-eslint/no-explicit-any
            skipSchemaErrors);
            body.push(executeInterface);
            const executeClass = await w.createExecuteClass(Client, Instance, QueryClient, ExecuteMsg, // eslint-disable-line  @typescript-eslint/no-explicit-any
            name, skipSchemaErrors);
